...
session_start();
$encryption_key = '';
if (isset($_SESSION) && isset($_SESSION['encryption_key']) && !empty($_SESSION['encryption_key']))
...
<script type="text/javascript">
	...
	function send_message(e) {
		...
		get_data({
			...
			is_encrypt: is_encrypt.value.trim()
		}, "send_message");
...
<script type="text/javascript">
	...
	function checkEncryptOptionChecked(cb) {
		if (cb.checked)
			$('#is_encrypt').val(1);
		else
			$('#is_encrypt').val(0);
	...
	function showActualMessage(e) {
		var encryption_key = '<?php echo $encryption_key; ?>';
		...
		Swal.fire({
			...
			preConfirm: (name) => {
				if (!name || name.trim() === "") {
					Swal.showValidationMessage("Please enter private key");
				} else if (name.trim() != encryption_key) {
					Swal.showValidationMessage("Private key is not valid");
				}
			},
		}).then((result) => {
			if (result.isDismissed) {
				...
				xml.onload = function() {
					if (xml.readyState == 4 || xml.status == 200) {
						...
						Swal.fire({
							//title: "Good job!",
							text: response.actual_message,
							allowOutsideClick: false,
							//icon: "success"
							imageUrl: "ui/icons/decrypt.png",
							imageWidth: 70,
							imageHeight: 70,
						});
					}
